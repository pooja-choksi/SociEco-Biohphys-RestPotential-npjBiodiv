...
#India district level spatial data
...
districts_as_df = as.data.frame(districts)
...
CLEAN THE DISTRCIT SPATIAL DATA TO MATCH THE DISTRICT LEVEL MPI AND OTHER SOCIO- ECONOMIC DATA 

```{r message=FALSE, warning=FALSE, include=TRUE}

#first check if all state names match 
state_names_missing =  setdiff(districts$NAME_1, district_mpi_summary$State)   # elements in first NOT in second

state_names_missing

#recode the state names to match the spatial data 
district_mpi_summary = district_mpi_summary%>%dplyr::mutate(State = recode(State, "Jammu And Kashmir" ="Jammu and Kashmir", 
...
district_names_missing =  setdiff(district_mpi_summary$District_Name, districts$NAME_2)   # elements in first NOT in second
...
"Dadra & Nagar Havel" = "Dadra and Nagar Haveli",
...
#9 districts missing becuase they're not in the dataset of land use data

```

COMBINE THE SPATIAL DATA WITH THE DISTRICT-MPI- CLIMATE DATA
...
#only MPI and land use data
writeOGR(obj = districts_merged, layer = "districts", dsn= "District-MPI-Merged-592districts.shp", driver = "ESRI Shapefile")
