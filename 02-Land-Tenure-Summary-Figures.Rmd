...
vulnerability$State[vulnerability$District == "Leh Ladakh"] <- "Jammu And Kashmir"
...
#Remove districts with NA for climate vulnerability index
district_mpi_climate = district_mpi_climate %>%
...
district_mpi_climate= district_mpi_climate%>%dplyr::mutate(Mean_Rank = rowMeans(.[, 28:29]))
...
#First rescale living standard of MPI. 
district_mpi_climate$Living_standards_scaled = district_mpi_climate$Living_standards/100 

#Calculate mean of living standards scaled and climate vulnerability
district_mpi_climate$People_centric_Mean= ((district_mpi_climate$Living_standards_scaled+ district_mpi_climate$Climate_vulnerability_index)/2)

#assign each district a rank
district_mpi_climate= district_mpi_climate%>%mutate(People_Centric_Rank = match(People_centric_Mean, unique(People_centric_Mean)))


```

SAVE THE CSV WHICH INCLUDES BOTH METHODS 
```{r}
...
QUESTION 2: STATE AND DISTRICT LEVEL TENURE CURVES 
