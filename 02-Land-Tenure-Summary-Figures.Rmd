...
COMBINE THE MPI DATA AND DAT_SUMMARY 
...
#we need to find duplicates if we want to make histograms - not needed to change for mapping
#find duplicate district names
...
state_names_missing =  setdiff(mpi_district_summary$State, vulnerability$State)   # elements in first NOT in second

state_names_missing 

#recode the state names to match the spatial data 
mpi_district_summary = mpi_district_summary%>%dplyr::mutate(State = recode(State, "Andaman Nicobar UT" = "Andaman And Nicobar Islands", "Jammu Kashmir" = "Jammu And Kashmir",
"West_Bengal"= "West Bengal", "Tamil_Nadu" = "Tamil Nadu"))

#change the state name of Leh and Ladakh in the mpi_district_summary as it has recently changed
mpi_district_summary$State[mpi_district_summary$District_Name == "Leh(Ladakh)"] <- "Ladakh"

mpi_district_summary$State[mpi_district_summary$District_Name == "Kargil"] <- "Ladakh"

#to avoid errors in renaming, change Raigarh in Maharashtra name outside recode code below 

mpi_district_summary = mpi_district_summary%>%mutate(District_Name = case_when(State == "Maharashtra" & District_Name == "Raigarh"~ "Raigad", TRUE ~ District_Name))
                                                     
#combine vulnerability and mpi_district-summary 
trial = merge(mpi_district_summary, vulnerability[c("District_Name", "State", "Climate_vulnerability_index", "NITI_District_code")], by.x = c("District_Name", "State"), by.y= c("District_Name", "State"))

#check missing districts 
missing = setdiff(vulnerability$District_Name, trial$District_Name) # elements in first NOT in second

#Recode district names in mpi_district_summary to match vulnerability 
mpi_district_summary = mpi_district_summary%>%dplyr::mutate(District_Name = recode(District_Name, 
"Uttar_Bastar_Kanker" = "Kanker",
"Subarnapur" = "Sonepur",
"Baudh" = "Boudh",
"Shupiyan" = "Shopian",
...
"Pashchimi_Singhbhum" = "West Singhbhum",
"Purbi_Singhbhum"= "West Singhbhum",                                "Saraikela_Kharsawan"= "Saraikela Kharsawan", 
"Dhar " = "Dhar", 
"North  District"= "North District",
"The_Nilgiris" = "The Nilgiris", 
"Kanshiram_Nagar" = "Kasganj", #new name 
"Sant_Kabir_Nagar" = "Sant Kabeer Nagar",
"Sant_Ravidas_Nagar_(Bhadohi)" = "Bhadohi",
"Dakshin_Dinajpur" = "Dinajpur Dakshin", 
"Koch_Bihar " = "Coochbehar",
"North_Twenty_Four_Parganas" = "24 Paraganas North",
"Paschim_Medinipur" = "Medinipur West",
"Purba_Medinipur" = "Medinipur East", 
"South_Twenty_Four_Parganas" = "24 Paraganas South",
"Baramula" = "Baramulla",
"Punch" = "Poonch",
"Firozpur" = "Ferozepur",
"Jalandar" = "Jalandhar",
...
"Udham_Singh_Nagar" = "Udam Singh Nagar",
"Uttar_Dinajpur" = "Dinajpur Uttar", 
...
#combine vulnerability and mpi_district-summary 
district_mpi_climate = merge(mpi_district_summary, vulnerability[c("District_Name", "State", "Climate_vulnerability_index", "NITI_District_code")], by.x = c("District_Name", "State"), by.y= c("District_Name", "State"))

#check missing districts 
missing = setdiff(mpi_district_summary$District_Name, vulnerability$District_Name) # elements in first NOT in second
