...
districts_data = merge(districts_data, mpi_district[c("District", "State", "MPI_District", "Number_of_MPI_PeopleDistrict", "Living_standards")], by.x = c("State", "District"), by.y= c("State", "District"))
...
trial = merge(dat.summary, mpi_district[c("District", "State", "MPI_District", "Number_of_MPI_PeopleDistrict", "Living_standards")], by.x = c("State", "District_Name"), by.y= c("State", "District"))
...
#Join mpi and dat.summary using state and district names
mpi_dat.summary = merge(dat.summary, mpi_district[c("District", "State", "MPI_District", "Number_of_MPI_PeopleDistrict", "Living_standards")], by.x = c("State", "District_Name"), by.y= c("State", "District"))
...
#there are 2 "Karbi Anglong' districts in Assam, remove one 
wrong_karbi = vulnerability%>%filter(District == "Karbi Anglong" & District_Code == "710")

vulnerability = anti_join(vulnerability, wrong_karbi)
...
#check missing districts 
missing = setdiff(vulnerability$District, district_mpi_climate$District_Name) # elements in first NOT in second
...
CREATE A RANK BASED ON MPI AND CLIMATE VULNERABILITY  
...
#rearrange the districts in descending order of MPI living standard section
district_mpi_climate  <- district_mpi_climate [order(district_mpi_climate$Living_standards, decreasing=TRUE),]

#assign each district a rank
district_mpi_climate= district_mpi_climate%>% mutate(MPI_rank = match(Living_standards, unique(Living_standards)))
...
#NOT INCLUDING TOTAL POPULATION IN THE ANALYSIS
#Total population
#rearrange the districts in descending order of MPI
...
district_mpi_climate= district_mpi_climate%>%dplyr::mutate(Mean_Rank = rowMeans(.[, 27:28]))
...
vars(Total_Indiv, Total_Common, Total_Forest, Total_Unaccounted,Total_Population, Total_hectares_for_prop),
.funs =c(sum = "sum"))

#rearrange tenure common in descending order
state_summary <- state_summary[order(state_summary$Total_hectares_for_prop_sum, decreasing=TRUE),]
...
state_hectares_tenure  = state_summary%>%mutate(State = factor(State, levels= prop_order$State, ordered =TRUE))%>%
  ggplot(aes(x = State, y = Total_hectares_for_prop_sum))+ 
  geom_col(aes(x = State, y = Total_hectares_for_prop_sum), fill = "#D3D3D3") + 
...
state_hectares_tenure


##DISTRICT LEVEL PLOT

#rearrange tenure common in descending order
dist_rearranged <- district_mpi_climate_summary[order(district_mpi_climate_summary$Total_hectares_for_prop_sum, decreasing=TRUE),]

#add cumulative column to the data frame
dist_rearranged[,"cum_indiv_hectares"] <- cumsum(dist_rearranged$Total_Indiv_sum)

dist_rearranged[,"cum_common_hectares"] <- cumsum(dist_rearranged$Total_Common_sum)

dist_rearranged[,"cum_forest_hectares"] <- cumsum(dist_rearranged$Total_Forest_sum)

dist_rearranged[,"cum_unaccounted_hectares"] <- cumsum(dist_rearranged$Total_Unaccounted_sum)

dist_rearranged[,"cum_total_population"] <- cumsum(dist_rearranged$Total_Population_sum)

#find duplicate district names
duplicated_districts = 
dist_rearranged[duplicated(dist_rearranged$District_Name), ]

#rename the duplicate districts to be able to plot in descending 
dist_rearranged = dist_rearranged%>%mutate(District_Name = case_when(District_Name == "Aurangabad" & State == "Bihar" ~ "Aurangabad_2", District_Name == "Bijapur" & State == "Karnataka" ~ "Bijapur_2", District_Name == "Bilaspur" & State == "Himachal Pradesh" ~ "Bilaspur_2", District_Name == "Hamirpur" & State== "Himachal Pradesh"~ "Hamirpur_2", District_Name == "Pratapgarh" & State == "Rajasthan" ~ "Pratapgarh_2", District_Name== "Raigarh" & State == "Maharashtra" ~ "Raigarh_2", District_Name == "Ranchi" & State == "Madhya Pradesh" ~ "Ranchi_2", TRUE ~ District_Name))
...
district_tenure_curves  = dist_rearranged%>%mutate(District_Name = factor(District_Name, levels= prop_order$District_Name, ordered =TRUE))%>%
 ...
 #geom_line(aes(y= cum_total_population/scaleRight), group = 1, color = "black", size = 1.2)+ 
  ...
  geom_line(aes(y= cum_forest_hectares/scaleRight), group = 1, color = "#117733", size = 1.2)+ 
  #geom_point(aes(y= cum_forest_hectares/scaleRight), group = 1, color = "#117733")+
    scale_fill_manual(values = c("#aab539", "#5b70dd", "#d2a035", "#c13fa6", "#4fc271", "#d33680", "#4e8f2e", "#d27cde", "#41844a", "#dc375f", "#5dc3a3", "#b82d24", "#4ab4dd", "#e55744", "#517dc0", "#e37733", "#7956a3", "#9db269", "#a7487a", "#2e8871", "#cc5967", "#677328", "#a798e0", "#8c6d2c", "#db8ebb", "#a6572c", "#91567c", "#d79f66", "#9f494f", "#e3867c"))+ 
...
for(i in unique(state_summary_long$Land_Tenure)) {
        name <- paststate_total_plot = state_summary_long%>%
