...
#rescale the climat vulnerability index for only these 550 districts
district_mpi_climate$Climate_vulnerability_scaled = 
  range01(district_mpi_climate$Climate_vulnerability_index)

#Calculate mean of living standards scaled and climate vulnerability
district_mpi_climate$People_centric_Mean= ((district_mpi_climate$Living_standards_scaled+ district_mpi_climate$Climate_vulnerability_scaled)/2)
...
write.csv(district_mpi_climate, "District-MPI-Ranked-V2.csv")
...
district_tenure_curves  = dist_rearranged%>%mutate(District_Name = factor(District_Name, levels= prop_order$District_Name, ordered =TRUE))%>%
 ...
 #geom_line(aes(y= cum_total_population/scaleRight), group = 1, color = "black", size = 1.2)+ 
  ...
  theme_classic()+ theme(axis.title = element_text(size = 12), axis.text.x = element_blank(), axis.ticks.x = element_blank()) +theme(legend.position = "bottom", axis.text = element_text(size=10))
