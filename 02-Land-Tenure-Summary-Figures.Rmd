...
#the 41 conflict obs that remain are only metropolitan areas/ towns (with no land use data), other districts (e.g.: Arunachal Pradesh) with no land use data available from census 2011 records and one new district in UP (Basti) for which there is no land use data. 
```

Total N = 593 districts
...
#rename the 6 duplicate districts to be able to plot in descending 
...
In the chunk of code below we combine the MPI, district census info and the climate vulnerability data. At the end of the code, we write the csv, which we need for our spatial data in 06. 
...
#there should 14 districts missing. these districts are all urban and do not have land use data from census 2011

#there are 6 duplicate districts names with no vulnerability data - to remove 
...
#write csv 
setwd("C:\\Users\\pooja\\Documents\\GitHub\\Land-Tenure-Restoration-Potential\\Land-Tenure-Restoration-Potential")
write.csv(district_mpi_climate_summary, "District-Climate-MPI.csv")
