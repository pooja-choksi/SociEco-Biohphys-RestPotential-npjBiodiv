...
biophys_people_scale = ggplot(districts_info) + geom_jitter(data= districts_info, aes(x = strass_rank, y= Cmbnd_R, shape = tenure_max_cat, color = Clmt_v_, size = MPI_Dst), alpha = 0.5)+ theme_bw()+scale_color_viridis(option = "D")+
  scale_y_continuous(name = "District Rank according to people-centric restoration opportunity") + scale_x_continuous(name="District Rank according to biophysical restoration potential")+ 
   geom_vline(xintercept =280, linetype="dashed") + geom_hline(yintercept = 206.5, linetype="dashed")
...
districts_info = districts_info%>%mutate(People_Cat = case_when(
  Cmbnd_R<=109.2 ~ "High",
  Cmbnd_R>=109.2 & Cmbnd_R<208.1 ~ "Medium",
  Cmbnd_R>=208.1 & Cmbnd_R<305.8 ~ "Low",
  Cmbnd_R>=305.8 & Cmbnd_R< 413 ~"Very low",
  TRUE~ "NA"))
