...
#alternative csv 
districts_info = read.csv("District-MPI-Biophys-Ranked.csv")
...
#alternative districts shp file 
...
# SUPPLEMENTARY INFORMATION ON VARIABLES USED IN STUDY (Fig S1) 
```{r message=FALSE, warning=FALSE, include=TRUE}

#Correlation between climate vulnerability index and the living standards component of multidimensional poverty index
ggplot(data = districts_info, aes(x = Lvn__, y = Clmt_vln_1)) +
    stat_poly_line() +
    stat_poly_eq() +
    geom_point()+ theme_bw()+xlab("Living standards component of multidimensional poverty index")+ylab("Climate vulnerability index")
...
biophys_people_scale1 = ggplot(districts_info) + geom_jitter(data= districts_info, aes(x = Strassburg_Scaled_final, y= Lvng_s_, color = tenure_max_cat,),size = 3, alpha = 0.6)+theme_bw()+scale_color_manual(values = tenure_category, labels = c("Non forest commons", "Forest commons", "Private"))+ scale_y_continuous(name = "Poverty (Living standards)")+ scale_x_continuous(name="Biophysical restoration potential")+ geom_vline(xintercept =0.7420543, linetype="dashed") + geom_hline(yintercept =0.71645570, linetype="dashed")+ labs(color ="Dominant land tenure")
...
People_Percentile_Upper<=50 & Strassburg_Percentile_Upper>=50 ~ 2,
People_Percentile_Upper>=50 & Strassburg_Percentile_Upper<=50 ~ 3,
TRUE ~ 0))
...
Climate_Vul_Percentile_Upper<= 50 & Strassburg_Percentile_Upper>= 50 ~ 2,
Climate_Vul_Percentile_Upper>= 50 & Strassburg_Percentile_Upper<= 50 ~ 3,
TRUE ~ 0))
...
#S1: Table of districts ranked as per climate vulnerability scale. Here we show only the total districts from each state
clim_vul_states = districts_info%>%filter(Climate_Vul_Percentile_Upper == 100)
clim_states = clim_vul_states%>%group_by(NAME_1)%>%count()
write.csv(clim_states, "TableS1-ClimVul-States-Total.csv")

#S2: Table of districts ranked as per poverty % scale. Total of the districts per state
poverty_states = districts_info%>%filter(Poverty_Percentile_Upper == 100)
poverty_states = poverty_states%>%group_by(NAME_1)%>%count()
write.csv(poverty_states, "TableS2-Poverty-States-Total.csv")

#S3: Table of districts ranked as per people-centric scale. Total of districts per state 
people_centirc_states = districts_info%>%filter(People_Percentile_Upper == 100)
people_states = people_centirc_states%>%group_by(NAME_1)%>%count()
write.csv(people_states, "TableS3-People-States-Total.csv")

#S4: Table of aspirational districts in the top people-centricvrestoration scale 
aspirational_people_centric = districts_info%>%filter(People_Percentile_Upper == 100 & Aspirational_District == "Aspirational_district")

aspirational = aspirational_people_centric%>%group_by(NAME_1, NAME_2,Lvng_, Clmt_vlnrb)%>%count()
...
kerala= districts_info%>%filter(NAME_1 == "Kerala")
...
#1. Poverty and Strassburg agreement percentiles 
agreement = districts_info%>%filter(Poverty_Strassburg_Agreement>1)

agreement$Poverty_Percentile_Upper = as.factor(agreement$Poverty_Percentile_Upper)
